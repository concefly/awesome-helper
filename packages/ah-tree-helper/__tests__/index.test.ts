...
describe('walk', () => {
  ...
  it('done 参数', () => {
    ...
    new TreeHelper(list).walk('0', (t, { done }) => {
      tapMap[t.id] = index++;
  ...
  it('trace && isLeaf', () => {
    ...
    new TreeHelper(list).walk(t0.id, (t, { trace, isLeaf: _isLeaf }) => {
      traceMap[t.id] = trace;
...
describe('flatten', () => {
  it('normal', () => {
    ...
    const list = [t0, t0_0, t0_1, t0_0_0];
    const result = new TreeHelper(list).getAllTraceList(t0.id);
...
describe('reduceTraceList', () => {
  it('normal', () => {
    ...
    const result = TreeHelper.reduceTraceList(traceList);
